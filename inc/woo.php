    ...
    if ($product) {
        $defaults = array(
            ...
            'class'      => implode(
                ...
                array_filter(
                    array(
                        'product-add-cart-btn product-add-cart-btn-2 cart-button icon-btn button',
                        'product_type_' . $product->get_type(),
                        $product->is_purchasable() && $product->is_in_stock() ? 'add_to_cart_button' : '',
                        $product->supports('ajax_add_to_cart') && $product->is_purchasable() && $product->is_in_stock() ? 'ajax_add_to_cart' : '',
                    )
...
add_action('woocommerce_before_shop_loop_item_list', 'harry_product_list');

// harry_product_details
